...
main()
{
	...
	strcpy(student[size].address,"121 Heritage Lane");
	...
	strcpy(student[size].startDate,"00/00/0000");
	strcpy(student[size].address,"689 candy road");
	strcpy(student[size].birthDate,"02/20/2000");
	strcpy(student[size].endDate,"00/00/0000");
	student[size].id=1092832;
...
void searchName(info student[],int size)
{
	...
	for(x=0;x<size;x++)
	{
		if(strcmp(fName,student[x].firstName)==0&&strcmp(lName,student[x].lastName)==0)
		{
		  printStudent(&student[x],1);
	    }
	}
